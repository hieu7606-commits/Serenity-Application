...
public sealed class MovieRow : Row<MovieRow.RowFields>, IIdRow, INameRow
{
    ...
    // A movie's genres live in the MovieGenres linking table, one row per movie/genre pair, so this
    // is NotMapped: there is no GenreList column. LinkingSetRelation is what fills the list on
    // retrieve and, on save, inserts and deletes MovieGenres rows to match it - naming the linking
    // row, its column pointing back at this movie, and its column holding each genre id.
    // LookupEditor is declared here rather than on the form so every form showing it gets the
    // dropdown; Multiple turns it into a tag-style multi-select.
